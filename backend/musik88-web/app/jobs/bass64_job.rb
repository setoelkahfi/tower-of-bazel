class Bass64Job < ApplicationJob
  queue_as :default
  ...
  def perform(audio_file_id)
    ...
    result_bass_backing_track_filename = "bass-backing-track-#{file.source_file.blob.filename}.wav"
    result_bass_backing_track_path = "#{file_output_directory}/#{result_bass_backing_track_filename}"

    system("ffmpeg -i #{result_drums_path} -i #{result_vocals_path} -i #{result_other_path} -filter_complex amix=inputs=3:duration=longest -y '#{result_bass_backing_track_path}'")

    until File.exist?(result_bass_backing_track_path)
      system("echo 'We are waiting for #{result_bass_backing_track_path}...'")
      sleep 60
    end
    result_bass_backing_track_file = File.open(result_bass_backing_track_path)

    # We only save the result if user doesn't cancel the process
    # Ideally, we cancel the running process immediately
    if !file.audio_file_bass64_status.nil? && file.audio_file_bass64_status.cancelled?
      system("rm -rf #{file_output_directory}")
      return
    end

    result_bass_backing_track = BassBackingTrack.new(audio_file_id: file.id)
    result_bass_backing_track.source_file.attach(io: result_bass_backing_track_file, filename: result_bass_backing_track_filename)
    result_bass_backing_track.save
    
      # Cleanup Spleeter output folder
    system("rm -rf #{file_output_directory}")

    if file.audio_file_bass64_status.nil?
      AudioFileBass64Status
        .new(audio_file_id: audio_file_id, status: :done)
        .save
    elsif
      file
        .audio_file_bass64_status
        .done!
    end

    ActionCable.server.broadcast "Bass64Channel", {
      audio_file_id: file.id,
      message: MyController.render(partial: 'partial_bass64_row', locals: { audio_file: file} )
    }
